function getFiles() {
    ...
    //create 'concat', 'uglify' and cssmin tasks
    var task, folder, dest, concat_files = {}, min_files = {}, min_css_files = {};
    for (var k in config) {
        ...
        min_css_files[k + '_css'] = {
            src: folder + 'css/' + prefix + '-editable.css',
            dest: folder + 'css/' + prefix + '-editable.min.css'
        };
    }

    return {concat_files: concat_files, min_files: min_files, min_css_files: min_css_files};
...
//test on several jquery versions
            ...
            dist: {
                files: [
                    ...
                    //licences
                    {
                        ...
                        flatten: true,
                        dest: '<%= dist %>/',
                        src: ['LICENSE-MIT', 'README.md', 'CHANGELOG.txt']
                    }
                ]
            },
            inputs_ext: {
    ...
    // Default task.
    grunt.registerTask('default', ['clean', 'jshint', 'concat', 'cssmin', 'uglify', 'copy']);

    // alive server
    grunt.registerTask('server', 'connect:server:keepalive');

    // build
    grunt.registerTask('build', ['clean', 'jshint', 'concat', 'cssmin', 'uglify', 'copy']);
